...
class Registrasi extends CI_Controller {
	...
	private function _simpan() {
	    ...
	    $config = [
            	...
            	'charset'	=>'utf-8',
            	'protocol'	=>'smtp',
            	'smtp_host'	=>'mail.timkoding.com',
            	'smtp_user'	=>'perpol@timkoding.com',
            	'smtp_pass'	=>'perpol@123',
            	'smtp_crypto'=>'ssl',
            	'smtp_port'	=>'465',
            	'crlf'		=>"\r\n",
            	'newline'	=>"\r\n"
            ...
            $this->email->from('perpol@timkoding.com', 'Perpustakaan Online');
            
            // Email penerima
            $this->email->to($penerima,$nama); // Ganti dengan email tujuan
            
            // Lampiran email, isi dengan url/path file
            //$this->email->attach('');
            
            // $isipesan='<h4>Hi, ' .ucwords($this->input->post('nama')) . '</h4>Klik tombol di bawah ini untuk memverifikasi email anda. <p><a href="' . base_url() . 'user/login/verify?email=' . $this->input->post('email') . '&token=' . urlencode($token) . '" style="background-color:#44c767;border-radius:28px;border:1px solid #18ab29;display:inline-block;cursor:pointer;color:#ffffff;font-family:Times New Roman;font-size:17px;font-weight:bold;padding:9px 17px;text-decoration:none;text-shadow:0px 1px 0px #2f6627;" target="_blank">Verifikasi</a></p>';
            
            // Subject email
            $this->email->subject('Email Verfikasi');
            
            // Isi email
            $this->email->message('Hi, ' .ucwords($this->input->post('nama')) . ' Klik Link di bawah ini untuk memverifikasi email anda.  ' . base_url() . 'user/login/verify?email=' . $this->input->post('email') . '&token=' . urlencode($token) . '"');
